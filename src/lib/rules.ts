...
export const LANGUAGE_RULES: LanguageRule[] = [
  ...
  },
  {
    ...
    pattern:
      "\\b(?:trans|gender) (?:social )?contagion\\b|\\bsocial contagion\\b(?=.{0,40}\\b(?:trans|gender|dysphoria|pronoun))",
    category: "lgbtq",
    severity: "high",
  ...
  },
  {
    ...
    pattern:
      "\\b(?:genital |chemical )?mutilat(?:e|ion|ing)\\b(?=.{0,50}\\b(?:trans|gender|transition|dysphoria|puberty|blocker))|\\bchemical(?:ly)? castrat(?:e|ion|ing)\\b(?=.{0,50}\\b(?:trans|gender|puberty|blocker|minor|child))",
    category: "lgbtq",
    severity: "high",
  ...
  },
  {
    id: "erase-women",
    pattern:
      "\\beras(?:e|ing|ure) (?:of )?wom(?:an|en)\\b|\\bwom(?:an|en)['’]?s? erasure\\b",
    category: "lgbtq",
    severity: "medium",
  ...
  },
  {
    id: "men-in-dresses",
    pattern:
      "\\bmen in (?:dresses|skirts|women['’]?s? (?:spaces?|bathrooms?|toilets?|shelters?|sports?))\\b|\\bman in a dress\\b",
    category: "lgbtq",
    severity: "high",
  ...
  },
  {
    id: "gender-confused",
    pattern:
      "\\bgender[- ]confused\\b|\\bconfused about (?:their |his |her )?gender\\b",
    category: "lgbtq",
    severity: "high",
  ...
  // ── Coded / dogwhistle (on by default; many soft — people may not know) ─
  {
    id: "cultural-marxism",
    pattern:
      "\\bcultural marxism\\b|\\bcultural marxists?\\b|\\bcultural bolshevism\\b",
    category: "coded",
    severity: "high",
...
export function getRuleById(id: string): LanguageRule | undefined {
  return LANGUAGE_RULES.find((rule) => rule.id === id);
}

export function rulesByCategory(): Record<string, LanguageRule[]> {
  const grouped: Record<string, LanguageRule[]> = {};
